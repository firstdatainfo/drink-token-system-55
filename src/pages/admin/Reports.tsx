    ...
    queryFn: async () => {
      try {
        ...
        const {
          data: orders,
          error
        } = await supabase.from('orders').select('id, created_at, total_amount, status').gte('created_at', startDateIso).lte('created_at', endDateIso).order('created_at');
        if (error) {
      ...
      try {
        ...
        const {
          ...
          error
            ...
            payment_method,
            order_items(*)
          `).gte('created_at', startDateIso).lte('created_at', endDateIso).order('created_at');
    ...
    queryFn: async () => {
      try {
        ...
        const {
          ...
          `).gte('created_at', startDateIso).lte('created_at', endDateIso).eq('status', 'completed');
        if (error) {
        ...
        const categorySales = data.flatMap(order => order.order_items.map(item => ({
    ...
    queryFn: async () => {
      try {
        ...
        const {
          ...
          `).gte('orders.created_at', startDateIso).lte('orders.created_at', endDateIso).eq('orders.status', 'completed');
        if (error) {
        ...
        const formattedData = Object.values(productSales).sort((a: any, b: any) => b.revenue - a.revenue).map((item: any, index) => ({
          id: index + 1,
          ...item
        }));
  ...
  return <Layout>
      ...
      <Card className="mb-6">
          ...
          <CardTitle className="flex justify-between items-center">
            ...
            <Popover open={isFilterOpen} onOpenChange={setIsFilterOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="text-sky-50 bg-blue-500 hover:bg-blue-400">
                ...
                <div className="grid gap-4">
                  ...
                  {dateRange === "custom" && <>
                      <div className="grid gap-2">
                        ...
                        <Input id="startDate" type="date" value={format(startDate, 'yyyy-MM-dd')} onChange={e => setStartDate(new Date(e.target.value))} />
                      </div>
                      
                      <div className="grid gap-2">
                        <Label htmlFor="endDate">Data Final</Label>
                        <Input id="endDate" type="date" value={format(endDate, 'yyyy-MM-dd')} onChange={e => setEndDate(new Date(e.target.value))} />
                      </div>
                    </>}
        ...
        <CardContent>
          ...
          {dateRange === "custom" && <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label htmlFor="endDate">Data Final</Label>
                <Input id="endDate" type="date" value={format(endDate, 'yyyy-MM-dd')} onChange={e => setEndDate(new Date(e.target.value))} />
              </div>
      ...
      {isLoading ? <Card className="mb-6">
          ...
          {reportType === "sales" && salesData && <Card className="mb-6">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
                <CardTitle>Vendas Diárias</CardTitle>
                <ExportOptions exportData={salesData} filename="vendas_diarias" />
              </CardHeader>
              <CardContent>
                {salesData.length > 0 ? <div className="h-80">
                ...
                bottom: 5
                        ...
                        <Legend />
                        <Line type="monotone" dataKey="vendas" stroke="#0EA5E9" activeDot={{
                  r: 8
                }} name="Vendas (R$)" />
                      </LineChart>
                    </ResponsiveContainer>
          ...
          {reportType === "categories" && categoryData && <Card className="mb-6">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
                <CardTitle>Vendas por Categoria</CardTitle>
                <ExportOptions exportData={categoryData} filename="vendas_por_categoria" />
              </CardHeader>
              <CardContent>
                {categoryData.length > 0 && categoryData[0].name !== 'Sem dados' ? <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
          ...
          {reportType === "products" && productData && <Card className="mb-6">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
              ...
              <CardContent>
                {productData.length > 0 ? <Table>
                    ...
                    <TableBody>
                      {productData.map((item: any) => <TableRow key={item.id}>
      ...
      <div className="mt-6">
        <ReportPDFGenerator productsData={productsData} salesData={rawSalesData} />
      </div>
    </Layout>;
};
