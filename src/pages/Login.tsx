...
const Login = () => {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            {isRegister ? "Crie uma nova conta de administrador" : "Entre com suas credenciais de administrador"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin}>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="seu.email@exemplo.com" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="password">Senha</Label>
                <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} />
              </div>
              <Button type="submit" className="w-full bg-pdv-blue hover:bg-pdv-blue/90" disabled={isLoading}>
                {isLoading ? isRegister ? "Registrando..." : "Entrando..." : isRegister ? "Registrar" : "Entrar"}
              </Button>
              ...
              {debugMode && <div className="mt-4 p-2 bg-gray-100 rounded text-xs font-mono text-gray-800 max-h-40 overflow-y-auto">
                  ...
                  {debugMessage.split('\n').map((line, i) => line ? <div key={i} className="whitespace-pre-wrap">{line}</div> : null)}
                </div>}

              {isRegister && email === "rodrigodev@yahoo.com" && <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">
        ...
        <CardFooter className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-center w-full gap-2">
            <Button variant="outline" className="bg-green-500 text-white hover:bg-green-600" onClick={handleVote}>
              <ThumbsUp className="mr-2" /> Votar ({voteCount})
            </Button>
            <Button variant="outline" className="bg-gray-100 text-pdv-blue hover:bg-gray-200" onClick={handleGoHome}>
      ...
      <div className="fixed bottom-4 right-4 flex gap-2">
        <Button variant="outline" size="sm" className={`text-xs ${debugMode ? "bg-blue-100" : "bg-gray-100"}`} onClick={toggleDebugMode}>
          {debugMode ? "Desativar Depuração" : "Ativar Depuração"}
        </Button>
        
        {debugMode && <Button variant="outline" size="sm" className="text-xs bg-gray-100" onClick={() => setDebugMessage("")}>
            Limpar logs
          </Button>}
      </div>
    </div>;
