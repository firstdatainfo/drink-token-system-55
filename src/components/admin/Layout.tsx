...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 relative">
      ...
      {/* Sidebar */}
      <div
        className={`fixed lg:static z-40 h-screen w-64 bg-white border-r shadow-sm transition-transform duration-300 overflow-auto ${
      ...
      >
        ...
        <nav className="px-3 mt-4 space-y-1">
          ...
          <div className="relative">
            ...
            {isSettingsOpen && (
              <div className="pl-8 mt-1 space-y-1 border-l-2 border-gray-200 ml-3">
                {integrationOptions.map((option) => (
                  <Link
                    key={option.path}
                    to={option.path}
                    className={`flex items-center px-3 py-2 rounded-md text-xs lg:text-sm transition-colors ${
